...
class MyNet2(nn.Module):
    def __init__(self, my_pretrained_model):
        ...
        self.top_layers = nn.Sequential(nn.Linear(1000, 200), nn.ReLU(), nn.Linear(200, 1))
...
def get_model():
    ...
    model.apply(initialize_weights_linear)
...
def train_epoch(resnet, device, dataloader, criterion, optimizer):
    ...
    resnet.train()
    for param in resnet.pretrained.parameters():
...
def training(resnet, data_dir, epochs, loss_fn, learning_rate, batch_size, gene):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < valid_loss_min or val_corr > valid_corr_max:
            ...
            model_save = data_dir + date + "_single_" + "_" + str(learning_rate) + "resnet2.pt"
