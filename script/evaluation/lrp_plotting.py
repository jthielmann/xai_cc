...
def plot_lrp(model, data, run=None, save_dir: str | None = None):
    ...
    for idx, sample in enumerate(data):
        ...
        patient = sample[2] if isinstance(sample, (tuple, list)) and len(sample) >= 3 else None
        tile = sample[3] if isinstance(sample, (tuple, list)) and len(sample) >= 4 else None
        ...
        if run is not None:
            ...
            run.log({f"lrp/attribution_{idx}": wandb.Image(panel, caption=caption)}, commit=True)
        if save_dir:
    ...
    if run is not None and len(table_rows) > 0:
        cols = ["idx", "patient", "tile", "pred_mean", "rel_abs_sum", "pos_frac", "coverage@10%", "panel"]
        table = wandb.Table(columns=cols)
        for r in table_rows:
            table.add_data(r["idx"], r["patient"], r["tile"], r["pred_mean"], r["rel_abs_sum"], r["pos_frac"], r["coverage@10%"], r["panel"])
